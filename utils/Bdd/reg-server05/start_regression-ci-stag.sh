...
features=SellForm
featuretags=@sellCiStag
ec2PublicIP=54.251.144.94
...
sudo docker cp imgs/playstation.jpeg athena-selenium-0-chrome-debug-2:/tmp/playstation.jpeg
sudo docker cp imgs/playstation2.jpeg athena-selenium-0-chrome-debug-2:/tmp/playstation2.jpeg

#run bdd feature tests
$HOME_DIR/athena/athena php --php-version=7.1 bdd . athena.bdd.ci-staging.json --browser=chrome --tags=$featuretags --parallel-process=$parallelism --parallel-features=$parallelism --athena-no-logo --no-colors --rerun
sleep 30s
